...
const Categories = () => {
  ...
  const handleAlta = () => {
    navigate("/categorias/alta");
  };

  const goToEditar = (id,email) => {
    localStorage.setItem("EditCategory",id);
    navigate("/categorias/editar");
  }

  const goToBorrar = async (id,nombre) => {
    localStorage.setItem("DeleteCategory",id);
    localStorage.setItem("DeleteCategoryName",nombre);
    navigate("/categorias/borrar");
  }
